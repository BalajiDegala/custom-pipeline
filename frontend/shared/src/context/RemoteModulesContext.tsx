...
export const RemoteModulesProvider = ({ children, skip }: Props) => {
  ...
  // MODIFIED: Safely access GlobalContext with fallback to avoid errors
  let siteInfo = null
  let isLoadingInfo = false
  try {
    const globalContext = useGlobalContext()
    siteInfo = globalContext.siteInfo
    isLoadingInfo = globalContext.isLoading?.siteInfo || false
  } catch (error) {
    console.warn('GlobalContext not available in RemoteModulesProvider, using defaults')
  }
