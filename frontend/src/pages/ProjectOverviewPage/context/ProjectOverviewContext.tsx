...
export const ProjectOverviewProvider = ({ children, modules }: ProjectOverviewProviderProps) => {
  ...
  const {
    ...
    // For attribute filters, we need to apply them to both task and folder scopes
    // Check if the sliceFilter is an attribute filter
    const isAttributeFilter = sliceFilter?.id?.startsWith('attrib.')
    
    if (isAttributeFilter && sliceFilter) {
      // Return the same filter for both task and folder
      return {
        task: [sliceFilter],
        folder: [sliceFilter],
      }
